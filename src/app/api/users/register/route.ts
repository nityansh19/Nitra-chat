...
export async function POST(request: Request) {
  try {
    ...
    await connectDB();
    const existing = (await User.findOne({ $or: [{ email }, { phone }] }).lean()) as { email: string } | null;
    if (existing) {
      return NextResponse.json(
        { error: `An account already exists for this ${existing.email === email ? "email" : "phone number"}.` },
        { status: 409 }
      );
    }
