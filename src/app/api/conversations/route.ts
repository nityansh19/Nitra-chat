...
export async function GET() {
  try {
    ...
    if (!userId || !mongoose.isValidObjectId(userId)) return NextResponse.json({ error: "Unauthorized." }, { status: 401 });
    await connectDB();
...
export async function POST(request: Request) {
  try {
    ...
    let conversation = await Conversation.findOne({ type: "direct", participants: { $all: [userId, other._id], $size: 2 } });
    if (!conversation) conversation = await Conversation.create({ type: "direct", participants: [userId, other._id] });

    await conversation.populate("participants", "name email phone nitraId initials bio status avatarUrl");
