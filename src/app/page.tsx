...
function Home() {
  ...
  const visible = useMemo(() => chats.filter(c => { const match = `${c.contact.name} ${c.contact.nitraId} ${c.messages.map(m => m.text).join(" ")}`.toLowerCase().includes(query.toLowerCase()); if (tab === "starred") return match && c.pinned; if (tab === "archive") return match && c.archived; if (tab === "saved") return match && c.messages.some(m => saved.includes(m.id)); return match && !c.archived; }), [chats, query, tab, saved]);
...
<div className="relative flex h-full min-h-0 gap-3 p-2 sm:p-3 lg:p-4">
    <aside className="desktop-rail glass hidden w-[68px] shrink-0 flex-col items-center justify-between rounded-[24px] py-3 md:flex"><div className="space-y-2"><button onClick={() => { setTab("inbox"); setActiveId(null); }} className="mb-4 flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-black"><MessageCircle size={21} /></button><IconButton label="Inbox" active={tab === "inbox"} onClick={() => setTab("inbox")}><MessageCircle size={18} /></IconButton><IconButton label="Starred" active={tab === "starred"} onClick={() => setTab("starred")}><Star size={18} /></IconButton><IconButton label="Saved" active={tab === "saved"} onClick={() => setTab("saved")}><Bookmark size={18} /></IconButton><IconButton label="Archive" active={tab === "archive"} onClick={() => setTab("archive")}><Archive size={18} /></IconButton></div><Link href="/profile"><Avatar initials={user.initials || "NU"} size="sm" online /></Link></aside>
    <section className={`${mobileList ? "fixed inset-2 z-50" : "hidden md:flex"} glass w-full max-w-[340px] shrink-0 flex-col overflow-hidden rounded-[24px]`}><div className="border-b border-white/[.06] p-4"><div className="flex items-center justify-between"><div><p className="text-[10px] uppercase tracking-[.2em] text-white/25">Nitra</p><h1 className="mt-1 text-lg font-semibold">Messages</h1></div><Link href="/connections" aria-label="Add friends" title="Add friends" className="flex h-9 w-9 items-center justify-center rounded-xl text-white/35 transition hover:bg-white/[.07] hover:text-white"><UserPlus size={18} /></Link></div><div className="mt-4 flex h-10 items-center gap-2 rounded-xl border border-white/[.06] bg-black/20 px-3"><Search size={14} className="text-white/20" /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search conversations" className="min-w-0 flex-1 bg-transparent text-xs outline-none placeholder:text-white/20" /></div><div className="mt-3 flex gap-1 overflow-auto">{(["inbox","starred","saved","archive"] as Tab[]).map(t => <button key={t} onClick={() => setTab(t)} className={`shrink-0 rounded-lg px-2.5 py-1.5 text-[9px] ${tab === t ? "bg-white/10" : "text-white/25"}`}>{t[0].toUpperCase()+t.slice(1)}</button>)}</div></div><Link href="/connections" className="mx-3 mt-3 flex items-center gap-3 rounded-2xl border border-cyan-300/10 bg-cyan-300/[.035] p-3 transition hover:border-cyan-300/20 hover:bg-cyan-300/[.06]"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-cyan-300/[.08] text-cyan-200/75"><UserPlus size={15} /></span><span className="min-w-0 flex-1"><span className="block text-xs font-semibold text-white/75">Add friends</span><span className="mt-0.5 block truncate text-[10px] text-white/25">Find people, send requests, accept or decline.</span></span><ChevronRight size={14} className="text-white/20" /></Link><div className="min-h-0 flex-1 overflow-auto p-2">{visible.map(c => <button key={c.id} onClick={() => openChat(c.id)} className={`flex w-full items-center gap-3 rounded-2xl p-3 text-left ${activeId === c.id ? "bg-white/[.075]" : "hover:bg-white/[.04]"}`}><Avatar initials={c.contact.initials} online={c.contact.online} /><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="truncate text-sm font-medium">{c.contact.name}</p>{c.pinned && <span className="text-white/25"><ChevronRight size={11} /></span>}{c.muted && <BellOff size={11} className="text-white/25" />}</div><p className="truncate text-[11px] text-white/30">{c.messages.at(-1)?.text || "Start a conversation"}</p></div>{c.unread > 0 && <span className="rounded-full bg-white px-2 py-1 text-[9px] font-bold text-black">{c.unread}</span>}</button>)}{!visible.length && <div className="p-8 text-center text-xs text-white/25">No conversations here.</div>}</div><div className="border-t border-white/[.06] p-3"><div className="flex items-center gap-3"><Avatar initials={user.initials || "NU"} size="sm" online /><div className="min-w-0 flex-1"><p className="truncate text-xs">{user.name}</p><p className="text-[10px] text-emerald-300/60">Available</p></div><Link href="/profile"><Settings size={15} className="text-white/25" /></Link></div></div></section>
    <section className="glass relative flex min-w-0 flex-1 flex-col overflow-hidden rounded-[24px]"><header className="flex h-[68px] shrink-0 items-center gap-3 border-b border-white/[.06] px-3 sm:px-5"><button onClick={() => setMobileList(true)} className="md:hidden"><Menu size={19} /></button>{active ? <><Avatar initials={active.contact.initials} online={active.contact.online} /><button onClick={() => setDetails(v => !v)} className="min-w-0 flex-1 text-left"><p className="truncate text-sm font-semibold">{active.contact.name}</p><p className="truncate text-[10px] text-emerald-300/60">{active.contact.online ? "Online now" : active.contact.status || "Available"}</p></button><IconButton label="Voice call" onClick={() => notify("Voice calls are coming with realtime.")}><Phone size={17} /></IconButton><IconButton label="Video call" onClick={() => notify("Video calls are coming with realtime.")}><Video size={17} /></IconButton><IconButton label="Details" active={details} onClick={() => setDetails(v => !v)}><Info size={17} /></IconButton></> : <div className="flex-1"><p className="text-sm font-semibold">Nitra Chat</p><p className="text-[10px] text-white/25">Choose a conversation</p></div>}</header>{active ? <><div ref={chatRef} className="min-h-0 flex-1 overflow-y-auto px-3 py-5 sm:px-8"><div className="mx-auto max-w-3xl"><div className="mb-8 flex items-center gap-3"><span className="h-px flex-1 bg-white/[.05]" /><span className="text-[9px] uppercase tracking-[.2em] text-white/20">Today</span><span className="h-px flex-1 bg-white/[.05]" /></div><div className="mb-8 text-center"><Avatar initials={active.contact.initials} size="lg" online={active.contact.online} /><h2 className="mt-4 text-lg font-semibold">{active.contact.name}</h2><p className="mt-1 text-xs text-white/25">{active.contact.nitraId}</p></div><div className="space-y-3">{active.messages.map(m => <MessageBubble key={m.id} message={m} onReact={emoji => setChats(p => p.map(c => c.id === active.id ? { ...c, messages: c.messages.map(x => x.id === m.id ? { ...x, reactions: x.reactions?.includes(emoji) ? x.reactions.filter(r => r !== emoji) : [...(x.reactions || []), emoji] } : x) } : c))} onSave={() => { setSaved(p => p.includes(m.id) ? p.filter(id => id !== m.id) : [...p, m.id]); notify(saved.includes(m.id) ? "Removed from saved" : "Saved message"); }} onDelete={() => setChats(p => p.map(c => c.id === active.id ? { ...c, messages: c.messages.filter(x => x.id !== m.id) } : c))} />)}</div></div></div><div className="shrink-0 px-3 pb-3 sm:px-5 sm:pb-5"><div className="mx-auto max-w-3xl"><AnimatePresence>{picker && <EmojiPicker onSelect={value => { if (value.startsWith("[GIF]")) send(value); else setMessage(current => `${current}${current ? " " : ""}${value}`); }} onClose={() => setPicker(false)} />}</AnimatePresence><div className="relative rounded-2xl border border-white/[.08] bg-white/[.035]"><div className="flex items-end gap-1 p-2"><IconButton label="Attach" onClick={() => notify("Attachments will be connected without paid Storage.")}><Paperclip size={17} /></IconButton><IconButton label="Emoji and GIFs" active={picker} onClick={() => setPicker(v => !v)}><Smile size={18} /></IconButton><textarea ref={inputRef} value={message} onChange={e => setMessage(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }} rows={1} placeholder={`Message ${active.contact.name.split(" ")[0]}…`} className="min-h-10 flex-1 resize-none bg-transparent px-2 py-2 text-sm outline-none placeholder:text-white/20" /><button onClick={() => send()} disabled={!message.trim()} className="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-black disabled:bg-white/[.07] disabled:text-white/20"><Send size={16} /></button></div></div><p className="mt-2 text-[9px] text-white/15">Enter to send · Shift + Enter for a new line</p></div></div></> : <div className="flex min-h-0 flex-1 items-center justify-center p-6 text-center"><div><Sparkles size={30} className="mx-auto text-white/40" /><h2 className="mt-5 text-2xl font-semibold">Your conversations start here.</h2><p className="mt-2 text-sm text-white/25">Pick someone from the left or start a new conversation.</p><button onClick={() => notify("Use the search/new conversation controls to connect.")} className="mt-6 rounded-xl bg-white px-4 py-2 text-xs font-semibold text-black">Start exploring</button></div></div>}</section>
    {details && active && <aside className="hidden w-[260px] shrink-0 flex-col overflow-auto rounded-[24px] border border-white/[.06] bg-white/[.025] p-5 xl:flex"><div className="flex justify-between"><p className="text-xs font-semibold uppercase tracking-[.18em] text-white/25">Context</p><IconButton label="Close" onClick={() => setDetails(false)}><X size={15} /></IconButton></div><div className="mt-8 text-center"><Avatar initials={active.contact.initials} size="lg" online={active.contact.online} /><p className="mt-4 text-sm font-semibold">{active.contact.name}</p><p className="mt-1 text-[10px] text-white/25">{active.contact.nitraId}</p><p className="mt-4 text-xs leading-5 text-white/30">{active.contact.bio || "A Nitra connection. Good conversations start here."}</p></div><div className="mt-7 space-y-2"><button onClick={() => { updateChat({ pinned: !active.pinned }); notify(active.pinned ? "Unpinned" : "Pinned"); }} className="flex w-full items-center gap-3 rounded-xl border border-white/[.05] p-3 text-left text-xs text-white/40"><span><Pin size={14} /></span>{active.pinned ? "Unpin conversation" : "Pin conversation"}</button><button onClick={() => { updateChat({ muted: !active.muted }); notify(active.muted ? "Notifications on" : "Muted"); }} className="flex w-full items-center gap-3 rounded-xl border border-white/[.05] p-3 text-left text-xs text-white/40"><BellOff size={14} />{active.muted ? "Turn notifications on" : "Mute notifications"}</button></div></aside>}
  </div>{toast && <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="fixed bottom-5 left-1/2 z-[80] -translate-x-1/2 rounded-full border border-white/10 bg-[#15171e]/95 px-4 py-2.5 text-xs text-white/70 shadow-xl">{toast}</motion.div>}
  </main>;
}
