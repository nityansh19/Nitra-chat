...
function Avatar({ initials, online = false, large = false }: { initials: string; online?: boolean; large?: boolean }) {
  return (
    <div className={`relative flex shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-gradient-to-br from-violet-400/30 via-indigo-500/15 to-cyan-300/10 font-semibold text-white shadow-[0_10px_35px_rgba(0,0,0,.22)] ${large ? "h-20 w-20 text-xl rounded-[24px]" : "h-12 w-12 text-xs"}`}>
      {initials}
      <span className="absolute inset-0 rounded-[inherit] ring-1 ring-inset ring-white/[.07]" />
...
export default function ConnectionsPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#07080c] text-white noise">
      ...
      <div className="relative mx-auto min-h-screen max-w-7xl px-4 py-5 sm:px-7 lg:px-10">
        <header className="flex items-center justify-between border-b border-white/[.06] pb-5">
          <div className="flex items-center gap-3">
            <Link href="/" className="group flex h-10 w-10 items-center justify-center rounded-xl border border-white/[.08] bg-white/[.025] text-white/45 transition-all duration-300 hover:-translate-x-0.5 hover:border-white/[.16] hover:bg-white/[.06] hover:text-white"><ArrowLeft size={17} /></Link>
            <div><p className="text-[9px] uppercase tracking-[.28em] text-cyan-300/45">Nitra / Social layer</p><h1 className="mt-1 text-lg font-semibold tracking-tight">People</h1></div>
          </div>
        ...
        <div className="grid gap-5 py-6 lg:grid-cols-[minmax(0,1fr)_340px]">
          <section className="min-w-0 rounded-[28px] border border-white/[.07] bg-white/[.025] p-5 shadow-[0_25px_90px_rgba(0,0,0,.22)] backdrop-blur-xl sm:p-7">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div><Pill><Users size={10} className="mr-1 inline" /> Discover</Pill><h2 className="mt-4 text-3xl font-semibold tracking-[-.045em] sm:text-4xl">{headline}</h2><p className="mt-2 text-sm text-white/30">Search by Nitra ID, name, email or phone. Your Nitra ID is the easiest way to find you.</p></div>
              <div className="hidden h-12 w-12 items-center justify-center rounded-2xl border border-violet-300/10 bg-violet-400/[.06] text-violet-200/70 sm:flex"><Sparkles size={18} /></div>
            </div>

            <div className="group relative mt-7">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-white/25 transition group-focus-within:text-cyan-300/65" size={18} />
              <input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search @nitra_id or name…" className="h-14 w-full rounded-2xl border border-white/[.08] bg-black/20 pl-12 pr-12 text-sm text-white outline-none transition-all duration-300 placeholder:text-white/20 focus:border-cyan-300/25 focus:bg-white/[.045] focus:shadow-[0_0_0_5px_rgba(34,211,238,.045),0_15px_50px_rgba(0,0,0,.18)]" />
              {query && <button onClick={() => setQuery("")} className="absolute right-4 top-1/2 -translate-y-1/2 rounded-lg p-1 text-white/25 transition hover:bg-white/[.07] hover:text-white"><X size={15} /></button>}
              {!query && <kbd className="absolute right-4 top-1/2 hidden -translate-y-1/2 rounded-lg border border-white/[.07] px-2 py-1 text-[9px] text-white/20 sm:block">TYPE TO SEARCH</kbd>}
            </div>

            <div className="mt-5 min-h-[390px]">
              <AnimatePresence mode="popLayout">
                {users.map((person, index) => (
                  <motion.button key={person.uid} layout initial={{ opacity: 0, y: 16, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1, transition: { delay: index * .035, type: "spring", stiffness: 380, damping: 28 } }} exit={{ opacity: 0, y: -10, scale: .98 }} whileHover={{ y: -2 }} whileTap={{ scale: .985 }} onClick={() => setSelected(person)} className="group mb-2 flex w-full items-center gap-4 rounded-2xl border border-white/[.06] bg-white/[.018] p-4 text-left transition-colors hover:border-white/[.12] hover:bg-white/[.045]">
                    <Avatar initials={person.initials} online />
                    <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{person.name}</p><p className="mt-1 truncate text-xs text-cyan-300/55">{person.nitraId}</p><p className="mt-1 truncate text-[11px] text-white/22">{person.bio || person.status || "Available"}</p></div>
                    <div className="flex items-center gap-2 text-white/20 transition-all group-hover:translate-x-0.5 group-hover:text-white/70"><span className="hidden text-[10px] sm:block">View</span><ChevronRight size={16} /></div>
                  </motion.button>
                ))}
              </AnimatePresence>
      ...
      <AnimatePresence>
        {selected && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-end justify-center bg-black/65 p-4 backdrop-blur-md sm:items-center" onClick={() => setSelected(null)}>
          <motion.div initial={{ opacity: 0, y: 30, scale: .96 }} animate={{ opacity: 1, y: 0, scale: 1, transition: { type: "spring", stiffness: 360, damping: 28 } }} exit={{ opacity: 0, y: 20, scale: .98 }} onClick={(e) => e.stopPropagation()} className="relative w-full max-w-md overflow-hidden rounded-[30px] border border-white/[.1] bg-[#101219] p-7 shadow-[0_35px_100px_rgba(0,0,0,.6)]">
            <div className="pointer-events-none absolute -right-24 -top-24 h-60 w-60 rounded-full bg-violet-500/[.09] blur-3xl" />
            <button onClick={() => setSelected(null)} className="absolute right-5 top-5 rounded-xl p-2 text-white/25 transition hover:bg-white/[.06] hover:text-white"><X size={17} /></button>
            <div className="relative flex items-center gap-4"><Avatar initials={selected.initials} online large /><div className="min-w-0"><Pill>Available</Pill><h3 className="mt-2 truncate text-xl font-semibold">{selected.name}</h3><p className="mt-1 truncate text-xs text-cyan-300/55">{selected.nitraId}</p></div></div>
            <p className="relative mt-6 rounded-2xl border border-white/[.06] bg-white/[.025] p-4 text-sm leading-6 text-white/35">{selected.bio || "Ready to connect on Nitra."}</p>
            <button disabled={creating} onClick={() => addPerson(selected)} className="relative mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-white text-sm font-semibold text-black transition hover:-translate-y-0.5 hover:bg-white/90 disabled:cursor-wait disabled:opacity-60">{added[selected.uid] ? <><Check size={16} /> Added · Open chat</> : <><MessageCircle size={16} /> {creating ? "Adding…" : "Add + start conversation">}</button>
            {added[selected.uid] && <Link href="/" className="relative mt-2 flex h-10 items-center justify-center rounded-xl border border-white/[.07] text-xs text-white/40 transition hover:bg-white/[.04] hover:text-white">Go to conversations <ChevronRight size={13} /></Link>}
          </motion.div>
        </motion.div>}
      </AnimatePresence>
    </main>
